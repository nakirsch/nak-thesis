...
outreg2 using "$output/sub1sum.xls", replace lab sum(log) ///
...
*comparison for pre-2015
keep if year < 2015
orth_out environmental_intensity_sales eff_estimate ln_ghg ln_gdp ln_pop ccpi_overall ///
 using "$output/sub1sum_compare_pre.xls", by(pledge_strong) replace overall se count compare test ///
 title("Comparison by Pledge Strength")
